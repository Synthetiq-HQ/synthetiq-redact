...
# Import the new NER engine
from ner_engine import UKPIIDetectionEngine
...
PARTIAL_REDACTION_POLICIES = {
    "phone": {
        "mode": "partial",
        "pattern": "XXX-XXX-{last4}",
        "mask": lambda val: _mask_phone(val),
    },
    "email": {
        "mode": "partial",
        "pattern": "{first2}***@{domain}",
        "mask": lambda val: _mask_email(val),
    },
    "address": {
        "mode": "partial",
        "pattern": "{street}, [REDACTED-town], {postcode}",
        "mask": lambda val: _mask_address(val),
    },
    "person_name": {
        "mode": "partial",
        "pattern": "{first_initial}. {last_name}",
        "mask": lambda val: _mask_name(val),
    },
    "nin": {"mode": "full"},
    "nhs_number": {"mode": "full"},
    "bank_account": {"mode": "full"},
    "dob": {"mode": "full"},
    "postcode": {"mode": "partial", "mask": lambda val: "[REDACTED-postcode]"},
    "email": {"mode": "partial", "mask": lambda val: _mask_email(val)},
    "default": {"mode": "full"},
}
...
def _mask_address(val: str) -> str:
    """Mask address keeping street number and postcode pattern."""
    # Keep the first number (street number)
    match = re.match(r"^(\d+\w*)\s+(.+)$", val.strip())
    if match:
        street_num = match.group(1)
        return f"{street_num} [REDACTED-address]"
    return "[REDACTED-address]"


def _mask_name(val: str) -> str:
    """Mask name to initial + last name."""
...
ROLE_VISIBILITY = {
    "caseworker": {
        ...
        "bank_account": "full",
        "dob": "full",
        "postcode": "none",         # Caseworker needs postcodes
        "default": "full",
    },
    "public_facing": {
        "phone": "full",
    ...
    "auditor": {
        "default": "none",  # Auditor sees everything
    },
    "admin": {
        "default": "none",
    },
...
def get_role_policy(role: str, redaction_type: str) -> str:
    """
    Get redaction mode for a role + type.
    Returns: "none" (visible), "partial" (masked), "full" (redacted)
    """
    role_config = ROLE_VISIBILITY.get(role, ROLE_VISIBILITY["processor"])
    return role_config.get(redaction_type, role_config.get("default", "full"))
...
class RedactionLearner:
    """Learn from human corrections to improve redaction accuracy."""
    
    def __init__(self, db_session=None):
        self.db = db_session
        self.corrections_cache = {}
    
    def record_correction(self, redaction_type: str, original_bbox: dict, 
                          corrected_bbox: dict, action: str):
        """
        Record a human correction.
        action: "expand", "shrink", "move", "approve", "reject"
        """
        if action in ["expand", "shrink", "move"]:
            # Calculate adjustment
            adj_x = corrected_bbox["x"] - original_bbox["x"]
            adj_y = corrected_bbox["y"] - original_bbox["y"]
            adj_w = corrected_bbox["w"] - original_bbox["w"]
            adj_h = corrected_bbox["h"] - original_bbox["h"]
            
            key = redaction_type
            if key not in self.corrections_cache:
                self.corrections_cache[key] = {"count": 0, "dx": 0, "dy": 0, "dw": 0, "dh": 0}
            
            self.corrections_cache[key]["count"] += 1
            self.corrections_cache[key]["dx"] += adj_x
            self.corrections_cache[key]["dy"] += adj_y
            self.corrections_cache[key]["dw"] += adj_w
            self.corrections_cache[key]["dh"] += adj_h
    
    def get_adjustment(self, redaction_type: str) -> dict:
        """Get learned adjustment for a redaction type."""
        if redaction_type not in self.corrections_cache:
            return {"dx": 0, "dy": 0, "dw": 0, "dh": 0}
        
        cache = self.corrections_cache[redaction_type]
        count = cache["count"]
        if count == 0:
            return {"dx": 0, "dy": 0, "dw": 0, "dh": 0}
    ...
    def apply_adjustment(self, bbox: list, redaction_type: str) -> list:
        """Apply learned adjustment to a bbox."""
        adj = self.get_adjustment(redaction_type)
        
        xs = [p[0] for p in bbox]
        ys = [p[1] for p in bbox]
        x_min = min(xs) + adj["dx"]
        x_max = max(xs) + adj["dx"] + adj["dw"]
        y_min = min(ys) + adj["dy"]
        y_max = max(ys) + adj["dy"] + adj["dh"]
        
        return [[x_min, y_min], [x_max, y_min], [x_max, y_max], [x_min, y_max]]


# Keep the rest of the existing redaction.py content below...
...
class RedactionEngine:
    ...
    def detect_titled_names(self, text: str) -> list[dict[str, Any]]:
        """
        Detect names preceded by titles (Mr, Mrs, Ms, Sir, Dr, Lord, Lady, etc.)
        in free-running text. Catches names that spaCy NER might miss on
        OCR-noisy handwriting.
        """
        spans: list[dict[str, Any]] = []
        # Match titles followed by 1-3 capitalized words
        pattern = re.compile(
            r"\b(Mr\.?|Mrs\.?|Ms\.?|Miss|Sir|Dr\.?|Prof\.?|Lord|Lady|Rev\.?|"
            r"Capt\.?|Major|Colonel|General|Dame)\s+"
    ...
    def detect_standalone_names(self, text: str) -> list[dict[str, Any]]:
        """
        Detect likely person names in letter headers and signatures.
        Looks for 2-3 capitalized words on their own line (common in signatures
        and letter closings). More aggressive than spaCy NER.
        """
        spans: list[dict[str, Any]] = []
        ...
        for raw_line in lines:
            ...
            offset += len(raw_line) + 1  # +1 for newline
            ...
            # Pattern: 2-3 capitalized words (e.g., "John Smith", "Mary Ann Jones")
            words = line.split()
            if 2 <= len(words) <= 4:
                if all(
                    w[0].isupper() and w[0].isalpha() and len(w) >= 2
                    for w in words
                    if w
                ):
                    # Avoid common non-name words
                    non_name_words = {"The", "From", "Date", "Subject", "Regarding",
                                      "Dear", "Yours", "Sincerely", "Regards", "Council",
                                      "Hillingdon", "London", "Monday", "Tuesday", "Wednesday",
    ...
    def detect_sensitive_text(
        ...
        for ns in self.detect_standalone_names(text):
            if not any(s["start"] < ns["end"] and s["end"] > ns["start"] for s in spans):
                spans.append(ns)
